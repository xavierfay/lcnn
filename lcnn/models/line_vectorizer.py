    ...
    def sample_lines(self, meta, jmap, joff, mode):
        with torch.no_grad():
            ...
            for i in range(n_type):
                ...
                # if mode != "training":
                #     K = min(int(above_threshold)+5, current_max_K)
                if mode != "training":
                    K = current_max_K
                else:
            ...
            padded_scores = [F.pad(s, (0, max_size - s.size(0)), value=float('nan')) for s in scores]
            padded_indices = [F.pad(idx, (0, max_size - idx.size(0)), value=float('nan')) for idx in indices]
